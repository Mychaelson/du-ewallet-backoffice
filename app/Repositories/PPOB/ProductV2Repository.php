...
class ProductV2Repository
{
    ...
    public function service($code){
        $data = DB::table('ppob.products_services')
        ->join('ppob.service_v2','ppob.service_v2.id','=','ppob.products_services.service_id')
        ->where('product_code',$code)
        ->select('ppob.service_v2.name as name_service','ppob.products_services.service_id','ppob.products_services.product_code','ppob.products_services.base_price')
        ->get();
    ...
    public function update($request , $code){
        ...
        $get_price = DB::table('ppob.products_services')
        ->where('product_code','=',$code)
        ->where('service_id',$request->service)
        ->first();
        $get_id = DB::table('ppob.products')
        ->where('code','=',$code)
        ->first();
