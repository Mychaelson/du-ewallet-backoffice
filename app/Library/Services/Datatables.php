...
class Datatables
{
    ...
    public function options(Array $opt)
    {
        ...
        $this->attributes = json_decode(json_encode([
            'data' => [
                ...
                'saveState' => $opt['saveState'] ?? FALSE
            ],
            'layout' => [
                'scroll' => $opt['scroll'] ?? TRUE,
                'footer' => $opt['footer'] ?? FALSE,
                'icons' => [
                    'rowDetail' => [
                        'expand' => "'flaticon2-down'",
                        'collapse' => "'flaticon2-next'"
                    ]
                ]
            ],
    ...
    public function column_details(Array $params = [])
    {
        $string = "$('<div/>').attr('id', 'child_data_local_'+d.data.".(isset($params['id']) ? $params['id'] : 'id').").appendTo(d.detailCell);
        $('#child_data_local_'+d.data.".(isset($params['id']) ? $params['id'] : 'id').").html(d.data.".(isset($params['details']) ? $params['details'] : 'details.betDetails').");";

        $this->attributes = array_merge((array) $this->attributes, ['detail' => '{ content : function(d){ '.trim(preg_replace('/\s+/', ' ', rtrim($string,';'))).'; } }']);
        return $this;
    }
                ...
                load: function () {
                    !(function () {
                        ...
                        e.on("datatable-on-ajax-fail", function (t,x) {
                            ...
                            alertModal.find(".modal-body").text(x.responseJSON ? x.responseJSON.message : "Whoops! Something went wrong.");
